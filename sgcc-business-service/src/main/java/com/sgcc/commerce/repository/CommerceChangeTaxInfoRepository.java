...
public class CommerceChangeTaxInfoRepository {
    ...
    {
        if (precompile) {
            String sql = "update b_change_taxticket set id =? ," +
                    "user_open_id = ?," +
    ...
    {
        if (precompile) {
            String sql = "delete from b_change_taxticket where id =?";
    ...
    public CommerceChangeTaxInfoDao findById(String id )
    {
        if (precompile) {
            String sql = "select id,user_open_id,new_install_company_name,new_install_address," +
                    ...
                    "invoice_date,invoice_img,new_install_apply_person,new_install_transactor,sq_idcard_positive_img," +
                    "sq_idcard_back_img,new_install_transactor_idcard,sq_attorney_img,new_install_transactor_tel,submit_date " +
                    "from b_change_taxticket where id =? ";

            try {
                return jdbcTemplate.queryForObject(sql,new Object[]{id}, new CommerceChangeTaxInfoRowMapper());
            } catch (Exception e) {
                e.printStackTrace();
                throw new RuntimeException("失败！！");
            }
    ...
    public List<CommerceChangeTaxInfoDao> findByOpenId(String openId )
    {
        if (precompile) {
            String sql = "select id,user_open_id,new_install_company_name,new_install_address," +
                    ...
                    "invoice_date,invoice_img,new_install_apply_person,new_install_transactor,sq_idcard_positive_img," +
                    "sq_idcard_back_img,new_install_transactor_idcard,sq_attorney_img,new_install_transactor_tel,submit_date " +
                    "from b_change_taxticket where user_open_id =?";

            try {
                return jdbcTemplate.query(sql,new Object[]{openId}, new CommerceChangeTaxInfoRowMapper());
            } catch (Exception e) {
                e.printStackTrace();
                throw new RuntimeException("失败！！");
            }
