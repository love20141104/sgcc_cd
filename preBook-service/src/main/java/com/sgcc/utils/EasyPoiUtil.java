...
public class EasyPoiUtil {
     ...
     * @param response
     */
    public static void exportMultipleSheetExcel(List<List<?>> list, String[] title, Class<?> pojoClass,
                                                String fileName, HttpServletResponse response) {
        int size = list.size();
        if (size==title.length){
            ...
            for (int i = 0; i < size; i++) {
                ...
                exportParams.setStyle(ExcelStyleUtil.class);    // 设置导出样式
                exportParams.setSheetName(title[i]);
                exportMap = new HashMap<>();
                exportMap.put("title",exportParams);
                exportMap.put("entity", pojoClass);
                exportMap.put("data",list.get(i));
